...
class SnakeGame:
    ...
    def do_training(self):
        ...
        for game in range(1, self.args.NUM_TRAIN_ITER + 1):
            # Reset the snake agent and board for a new game
            self.env.reset()  # Reset board state
            ...
            while not dead:
                # Step in the game environment with the selected action
                state, points, dead = self.env.step(action)
                
                # Get the action from the agent (in training mode, it will update Q-table)
                action = self.agent.agent_action(state, points, dead)


                # head_x, head_y, body, food_x, food_y = state
                # squares_visited.update(body)

                # Decay Epsilon
                #self.agent.Ne = max(self.agent.Ne * epsilon_decay, min_epsilon)
            # print("dead after", count, "moves")
            # if dead:
            #     print("dead for sure")
            # print("food is at", food_x, food_y)
            # print("snake length", len(body))
            # print("snake visited", squares_visited)
    ...
    def do_testing(self):
        ...
        for game in range(1, self.args.NUM_TEST_ITER + 1):
            ...
            while not dead:
                ...
                # Get the action from the agent (in testing mode, no Q-table updates)
